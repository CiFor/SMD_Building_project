...
if ( typeName(_arrayInput select 0) == "DISPLAY" ) then {
	TSW_DisplayID = (_arrayInput select 0);
};

if ( isNil("TSW_DisplayID") ) then {
	_display = findDisplay 632;
} else {
	_display = TSW_DisplayID;
};
...
if ( (count switchableUnits) <1 ) then { _mode = "CA_TS_Close"}; 
...
switch _mode do 
{
	case "CA_TS_Pause":
	{
		...
		//onMapSingleClick "_dummy =[_pos, ""CA_TS_MapClick""] call compile preprocessFile ""\ca\ui\scripts\TeamSwitch.sqf"" ";
		...
		_mapControls = [555,556,557,558,559,560,561];
		private ["_x"];	
		...
		debuglog  difficultyEnabled "Map";
		
		//Hide or show map according to last user-selected status
		if ((str TSW_mapDisplayed) == "false") then 
		{
			_dummy = [_pos, "CA_TS_ShowMap"]  call compile preprocessFile  "\ca\ui\scripts\TeamSwitch.sqf";
	...
	case "CA_TS_Resume":
	{
		...
		//may have undesired effect!
		//removing teamswitcgh script from the map control 	
		onMapSingleClick "";	
	};
 ...
 //lbSetCurSel [idc, index]
	...
	case "CA_TS_ShowMap":
	{
		...
		_arrowDown = "\Ca\ui\data\ui_map_arrow_close_ca.paa";
		_arrowUp = "\Ca\ui\data\ui_map_arrow_open_ca.paa";
		//Change icon
		_control = _display displayctrl 561;
		TSW_mapDisplayed = ctrlShown _control;
		
		
		if ((str TSW_mapDisplayed) == "true") then 
		{
	...
	case "CA_TS_ViewUnit":
	{
		...
		if ((str TSW_externalCamera) == "false") then 
		{
